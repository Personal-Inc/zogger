...
if (!ss.storage.report) {
 ss.storage.report = {};
 ss.storage.report.requests = [];
 ss.storage.report.facets = {};
 ss.storage.report.facets["fieldNameFacet"] = {};
}
...
function collectRequest(requestData){
  if (requestData.type == "formSubmit"){
    ...
    for (var k in data){
      incrementFacet("fieldNameFacet", k);
    }
  }
  requestData.timestamp = new Date().getTime();
  ss.storage.report.requests.push(requestData);
}

function reportToConsole(){
  for (var r in ss.storage.report.requests) {
    console.log(JSON.stringify(ss.storage.report.requests[r]));
  }
}
